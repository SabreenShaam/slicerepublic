...
class ClassesTestCase(TestCase):
    def setUp(self):
        ...
        setup_resources.setup_passport_studio_access()

    @skipIf(False, "Skip this test")
    def test_fetch_all_active_passport_services(self):
        list = PassportService.fetch_all_active_passport_services(self.mbo_client_home, self.external_studio)
        self.assertEquals(1, len(list))


class DummyRequest():
    def __init__(self, setting):
        self.user = setting.user_home
        self.auth = DummyAuth(setting)


class DummyAuth():
    def __init__(self, setting):
        self.studio = setting.home_studio
...
class SetupResources(BaseExternalTestSetup):
    ...
    def setup_mbo_client_services1(self):
        ...
        self.setting.mbo_client_service_4 = mommy.make_recipe('services.mbo_client_service', name='Intro',
                                                              mbo_client=self.setting.mbo_client_home,
                                                              mbo_client_service_id=100246934,
                                                              current=True,
                                                              count=5,
                                                              remaining=5,
                                                              payment_date=make_utc(datetime(2016, 12, 30)).date(),
                                                              active_date=make_utc(datetime(2016, 12, 30)).date(),
                                                              expiration_date=make_utc(datetime(2017, 12, 30)).date(),
                                                              program=self.program,
                                                              last_sync_date='2016-06-24 07:21:04',
                                                              )
    ...
    def setup_passport_studio_access(self):
        self.setting.passport_studio_access = mommy.make_recipe('services.passport_studio_access', studio=self.setting.external_studio,
                                                                mbo_service=self.setting.mbo_service)
